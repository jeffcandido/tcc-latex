...
Os \textit{container}'s são projetados para manter suas próprias visualizações contidas de namespaces e têm acesso limitado aos \textit{hosts} nos quais
são executados. Por padrão, os \textit{container}'s têm uma tabela de processos, interfaces de rede, sistemas de arquivos e recursos IPC separados do host.
Muitos recursos de segurança, como recursos e SELinux, são colocados em \textit{container}'s para controlar o acesso ao sistema host e outros
\textit{container}'s. Embora os \textit{container}'s possam usar recursos do host, os comandos executados a partir de um \textit{container} têm uma capacidade muito
limitada de interagir diretamente com o host.

Alguns \textit{container}'s, entretanto, têm como objetivo acessar, monitorar e, possivelmente, alterar recursos no sistema host diretamente. Eles são
chamados de \textit{container}'s superprivilegiados.

O Host deve ser enxuto. Muitas ferramentas que você pode desejar usar para gerenciar ou solucionar problemas do host não são incluídas por padrão.

A melhor maneira de adicionar ferramentas a um  Host é incluir essas ferramentas em um \textit{container}.

Você pode trazer um SPC para um Host, solucionar um problema e removê-lo quando não for mais necessário para liberar recursos.

Aqui estão os motivos pelos quais alguns \textit{container}'s precisam de privilégios extras:

Acesso ao soquete docker: a comunicação com o serviço docker por meio do soquete docker requer privilégios especiais.

Necessidade de carregar módulos do kernel: Qualquer \textit{container} que precise carregar módulos do kernel deve ter privilégios.

É necessário ignorar o SELinux: se um \textit{container} não funcionar com o SELinux no modo de aplicação, ele precisa de privilégios especiais
para escapar do confinamento do SELinux. O \textit{container} openshift3 / logging-fluentd é um exemplo de tal \textit{container}.
(Observe que é melhor criar uma política SELinux para o \textit{container} e rotular os arquivos que ele precisa acessar apropriadamente para que
possa funcionar com o SELinux.)

A próxima seção ilustra como os \textit{container} superprivilegiados são executados e como os recursos do host são acessados a partir de um SPC.
...
Para verificar se possui a chave com o \textit{fingerprint} \textit{Docker} (no momento em que este texto foi escrito,
